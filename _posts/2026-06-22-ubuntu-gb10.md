...
The naive fix - adding `sbsa_gwdt` to `/etc/modules-load.d/` - appears to work at provisioning time because `modprobe` bypasses soft blacklists directly. **It silently breaks on the first reboot.**

The `linux-nvidia-hwe` kernel package ships a per-kernel blacklist file at:

```
/usr/lib/modprobe.d/blacklist_linux-hwe-6.17_<uname-r>.conf
```

containing `blacklist sbsa_gwdt`. At every boot, `systemd-modules-load.service` sees this and silently skips loading the module. The blacklist is reinstalled with every kernel update.

### The fix

Load `sbsa_gwdt` from the **initramfs** - before systemd starts and before any `/usr/lib/modprobe.d/` blacklist is evaluated:

```bash
# Belt-and-suspenders: keep modules-load.d entry too
echo sbsa_gwdt | sudo tee /etc/modules-load.d/sbsa_gwdt.conf

# Add to initramfs (survives kernel updates - /etc/initramfs-tools/modules is not owned by any package)
...
`/etc/initramfs-tools/modules` is a static file not owned by any package. When a new kernel is installed, `update-initramfs` runs automatically and rebuilds the initramfs, pulling in this file - so the fix persists even when the new kernel reinstalls its own blacklist.

Verify it loaded:

```bash
lsmod | grep sbsa_gwdt          # should show the module
...
- Loads the SBSA watchdog driver from initramfs to bypass the kernel package blacklist (Step 2 fix)
