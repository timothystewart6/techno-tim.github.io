...
When you buy an NVIDIA DGX Spark or an ASUS Ascent GX10, it ships with DGX OS. DGX OS is NVIDIA's managed Ubuntu image, and it is fine - if you want a full GNOME desktop on an AI box, but that's not really what I was looking for.
...
| Canonical Kernel | 6.17.0-1032-nvidia (HWE) - `linux-nvidia-hwe-24.04` |
| NVIDIA GPU Driver | 580.178.04 (open kernel modules) |
...
> *Warning: The tarball ships a `dgx.sources` entry pointing at a `baseos/8/` path that 404s. If `apt update` fails on the DGX repo, rewrite `dgx.sources` to drop the `/8` and use the working components:*
{: .prompt-warning }
...
> *Info: `nvidia-system-extra` pulls in Docker CE and the NVIDIA Container Toolkit as dependencies. If you want to manage Docker installation separately, skip the next command and install Docker in Step 4 instead.*
{: .prompt-info }
...
Install the Linux perf tools and peermem loader. Pin the version-specific linux-tools that matches the nvidia HWE kernel (the `linux-tools-nvidia-hwe-24.04` metapackage now tracks the 7.x line):
...
Install and boot into the HWE kernel before installing the driver. Pin the validated 6.x line and block 7.x first, then install the specific kernel packages (the `linux-nvidia-hwe-24.04` metapackage now resolves to the 7.x line, and to nothing when 7.x is pinned):
...
> *Danger: Force the nvidia-container-runtime into legacy mode. With `nvidia-container-toolkit` 1.19+, the default `mode = "auto"` activates CDI mode and skips the legacy hook that populates the CUDA image entrypoint - CUDA containers then fail with `exec format error`. Force legacy mode so the hook always runs:*
{: .prompt-danger }
...
If you run a dedicated inference server, also add a weekly Docker prune so unused images and build cache get cleaned automatically (keep volumes - they hold model data):
...
PRUNE
sudo chmod 0755 /opt/actions-runner/prune-docker.sh
echo '0 3 * * 0 root /opt/actions-runner/prune-docker.sh >> /var/log/docker-prune.log 2>&1' | \
  sudo tee /etc/cron.d/docker-prune
```
...
Verify GPU access from inside a container. Using a CUDA image rather than a plain `ubuntu:24.04` also confirms the legacy-mode fix above works (otherwise you get an `exec format error`):
...
> *Tip: this step is now automated by the `doca_ofed` Ansible role - run `ansible-playbook site.yml --tags doca_ofed` instead of the commands above. The role detects the ConnectX-7 NIC (skipping itself if absent) and reboots automatically to load the new kernel modules. I added this after finding one of my two GX10s never got DOCA-OFED installed by hand, so it silently missed OFED/firmware updates that the other node picked up.*
{: .prompt-tip }
...
- Pins the validated 6.x kernel line (blocking 7.x), resolves the latest 6.x image, and installs the versioned kernel packages, then reboots
...
- Forces `nvidia-container-runtime` into legacy mode and verifies GPU access in a container
...
- Installs a weekly Docker prune cron
- Optionally queues MOK key enrollment if `mok_password` is provided

The `doca_ofed` role:

- Checks for a ConnectX-7 NIC via `lspci` and skips the whole role on hosts without one
- Installs `doca-bos8-latest-repo` and `nvidia-repo-keys`, then `apt full-upgrade`
- Installs `nvidia-system-mlnx-drivers`
- Reboots if the packages changed, to load the new `mlx5_core` DKMS driver and apply any pending NIC firmware update
- Asserts `mlx5_core` is loaded afterward
...
# (CUDA image also confirms the legacy-mode fix)
docker run --gpus=all --rm nvcr.io/nvidia/cuda:13.2.0-base-ubuntu24.04 nvidia-smi
